...
public class Router {
    ...
    public boolean checkBindingKey(String bindingKey) {
        ...
        // 因为前三种相邻的时候，实现匹配的逻辑会非常繁琐，同时功能性提升不大
        // 1. aaa.#.#.bbb   =>非法
        // 2. aaa.#.*.bbb   =>非法
        // 3. aaa.*.#.bbb   =>非法
        // 4. aaa.*.*.bbb   =>合法
    ...
    public boolean checkRoutingKey(String routingKey) {
        ...
        // 把每个字符都检查过，没有遇到非法情况，此时就直接返回 true
        return true;
    }
    ...
    private boolean routeTopic(Binding binding,Message message) {
        ...
        while (bindingIndex<bindingTokens.length && routingIndex<routingTokens.length) {
            if(bindingTokens[bindingIndex].equals("*")) {
                ...
                if(bindingIndex==bindingTokens.length) {
                    //[情况三] 该 # 后面没有东西了，说明此时一定匹配成功了
                    return true;
                }
                // [情况四] # 后面还有东西，拿着这个内容，去 routingKey 中往后找，找到对应的位置
                // findNextMatch 这个方法用来查找该部分在 routingKey的位置，返回该下标;没找到就返回-1
        ...
        // [情况五] 判定是否是双方同时到达末尾
        // 比如 aaa.bbb.cc 和 aaa.bbb 是要匹配失败的
